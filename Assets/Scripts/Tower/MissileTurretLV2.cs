...
{   
    ...
    protected override void FindTarget()
    {
        ...
        if (sortedTargets.Count > 0)//1,4,2,3 순서로 표적 설정
        {
            Target1 = sortedTargets[0].collider.transform;
            if(sortedTargets.Count<2)
                Target4 = sortedTargets[0].collider.transform;
        }
        if (sortedTargets.Count > 1)
        {
            if (Target4 == null)
            {
                Target4 = sortedTargets[1].collider.transform;
            }
        }
        if (sortedTargets.Count > 2)
        {
        ...
        if (sortedTargets.Count > 3)
        {
            if (Target3 == null)
            {
                Target3 = sortedTargets[3].collider.transform;
            }
        }
