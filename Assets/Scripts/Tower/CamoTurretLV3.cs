...
public class CamoTurretLV3 : MonoBehaviour
{   
    [Header("References")]
    ...
    private void Shoot()//총알 객체화 후 목표로 발사(FireRateController에서 수행)
    {
        ...
        bulletScript3.SetTarget(bulletFireDirection3);
    ...
    {
        float angle = Mathf.Atan2(_target.position.y - transform.position.y, _target.position.x - transform.position.x) * Mathf.Rad2Deg - 80f;
